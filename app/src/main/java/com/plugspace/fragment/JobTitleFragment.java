...
public class JobTitleFragment extends BaseFragment {
    Interfaces.OnGoBackClickListener backClickListener;
    Activity activity;
    Button btnNext;
    EditText etJobTitle;

    public JobTitleFragment(Interfaces.OnGoBackClickListener activity) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        initViewData(view);
        initClick();
        if (!Utils.isValidationEmpty(Constants.loginDataModel.getJobTitle())) {
            etJobTitle.setText(Constants.loginDataModel.getJobTitle());
            Logger.e("16/12 - slcJOb - >", Constants.loginDataModel.getJobTitle());
        }
        return view;
    }
    ...
    public void isValid() {
        ...
        if (Utils.isValidationEmpty(jobTitle)) {
            Utils.showAlert(activity, getString(R.string.app_name), getString(R.string.alert_enter_job_title));
        } else {
            Constants.loginDataModel.setJobTitle(jobTitle);
//            Logger.e("16/12", "job - " + etJobTitle.getText().toString().trim());
            if (backClickListener != null) {
                backClickListener.gobackClick(17);
            }
        }
    }
