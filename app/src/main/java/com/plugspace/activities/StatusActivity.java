...
public class StatusActivity extends BaseActivity implements StoriesProgressView.StoriesListener {
...
//                                storyView.resetStoryVisits();
                                storiesProgressView.setStoriesCount(lstStory.size());
                                storiesProgressView.setStoryDuration(5000L);
                                storiesProgressView.setStoriesListener(StatusActivity.this);
                                for (MediaModel mediaModel : lstStory) {
                                    String path = mediaModel.getMedia();
                                    pathList.add(path);
                                }
                                setupVideoPlayer();
                            }
...
//                    // TODO: 15/2/22 Confirm:
    ...
    private void setupVideoPlayer() {
        ...
        if (pathList.get(videoPathIndex).contains(".mp4")) {
            videoView.setVideoURI(Uri.parse(pathList.get(videoPathIndex)));
//            videoView.setVideoURI(Uri.parse("https://www.plugspace.io/public/story/165380772462931a6cee2a6.mp4"));
            ...
            Glide.with(StatusActivity.this)
                    .load(pathList.get(videoPathIndex))
                    .centerCrop()
        ...
        if (pathList.get(videoPathIndex).contains(".mp4")) {
            player.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
                public void onCompletion(MediaPlayer mp) {
                    ...
                    if (videoPathIndex == pathList.size()) {
                        return;
                    }
                    setupVideoPlayer();
                }
